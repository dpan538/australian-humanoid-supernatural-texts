#!/usr/bin/env python3
"""Collect card-ready public metadata records from Crossref.

Crossref is used here as a public bibliographic metadata source. The collector
does not fetch restricted full text. It only inserts records with enough public
metadata to render a review card and marks region data as reviewable signals.
"""
...
LOGGER = logging.getLogger("collect_crossref_metadata")

BASE_URL = "https://api.crossref.org/works"
SOURCE_NAME = "Crossref"
SOURCE_TYPE = "academic_metadata"
ROUND_NAME = "crossref_public_metadata_round_003"
USER_AGENT = "AustralianHumanoidPublicTexts/0.3 (mailto:research@example.com)"
RAW_TEXT_DIR = PROJECT_ROOT / "data" / "raw" / "text"
DEFAULT_STATUS_CSV = PROJECT_ROOT / "data" / "interim" / "crossref_round_003_candidates.csv"
DEFAULT_REPORT = PROJECT_ROOT / "data" / "processed" / "crossref_round_003_report.md"
...
TERM_SPECS = [
    ...
    TermSpec("Mamu", "Mamu", "Mamu Australia Aboriginal spirit", "AU", "Australia", "low", 2),
...
def list_text(value: Any) -> str:
    if isinstance(value, list):
        return clean_space(" ".join(str(item) for item in value if item))
...
def date_published(item: dict[str, Any], year: int) -> str:
    for field in ("published-print", "published-online", "published", "issued", "created"):
        parts = (item.get(field) or {}).get("date-parts") or []
        if parts and parts[0]:
            vals = [str(v) for v in parts[0] if v]
            if vals:
                return "-".join(vals)
    return str(year)
...
def item_url(item: dict[str, Any]) -> str:
    ...
    return clean_space(item.get("URL"))


def publication_name(item: dict[str, Any]) -> str:
    return list_text(item.get("container-title")) or list_text(item.get("publisher")) or "Crossref"


def authors(item: dict[str, Any], limit: int = 4) -> str:
    names: list[str] = []
    for author in item.get("author") or []:
        given = clean_space(author.get("given"))
        family = clean_space(author.get("family"))
        name = clean_space(f"{given} {family}")
        if name:
            names.append(name)
    if len(names) > limit:
...
def candidate_from_item(spec: TermSpec, item: dict[str, Any]) -> dict[str, Any] | None:
    title = list_text(item.get("title"))
    abstract = clean_space(item.get("abstract"))
    year = published_year(item)
    if not title or year is None:
        return None
    text = "\n".join([title, abstract, publication_name(item)])
    ...
    snippet_parts = [
        f"Public Crossref metadata record for a published work matching '{spec.term}'.",
        f"Work type: {item.get('type') or 'unknown'}.",
        f"Publication/source: {publication_name(item)}.",
        f"Location signal is figure-associated region ({region}), not a verified event place.",
    ]
    if abstract:
        snippet_parts.append("Abstract excerpt: " + abstract[:420])
    return {
        "status": "card_ready",
        "skip_reason": "",
        "figure": spec.figure,
        ...
        "year": year,
        "date_published": date_published(item, year),
        "title": title,
        "url": url,
        "external_id": "crossref:" + (clean_space(item.get("DOI")).lower() or clean_space(item.get("URL"))),
        "publication": publication_name(item),
        "author": authors(item),
...
def fetch_items(spec: TermSpec, rows: int, max_pages: int, delay: float) -> list[dict[str, Any]]:
    ...
    for page in range(max_pages):
        params = {
            ...
            "rows": rows,
            "offset": page * rows,
            "filter": "from-pub-date:1803-01-01,until-pub-date:2026-12-31",
            "select": "DOI,URL,title,container-title,publisher,author,issued,published,published-print,published-online,created,type,abstract",
        }
        url = BASE_URL + "?" + urlencode(params)
        LOGGER.info("Fetching %s offset %s", spec.figure, page * rows)
        response = None
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    LOGGER.warning("Skipping %s offset %s after request failure: %s", spec.figure, page * rows, exc)
                    return items
                time.sleep(delay + attempt)
        if response is None:
            return items
        batch = response.json().get("message", {}).get("items", [])
        if not batch:
            break
        items.extend(batch)
        if len(batch) < rows:
    ...
    if row:
        ...
        conn.execute(
            ...
            (
                ...
                "public_metadata_api",
                "public_metadata",
                "Crossref public metadata only; no restricted full text is collected.",
                source_id,
            ),
        )
        return source_id
...
def save_raw_text(record_id: int, row: dict[str, Any]) -> str:
    ...
    raw = {
        ...
        "note": "Crossref public metadata only; full text was not collected.",
    }
...
def existing_keys(conn) -> tuple[set[str], set[str], set[tuple[str, int]]]:
    external_ids = {row["external_id"] for row in conn.execute("SELECT external_id FROM records WHERE external_id LIKE 'crossref:%'") if row["external_id"]}
...
def insert_record(conn, row: dict[str, Any], source_id: int) -> int:
    ...
    conn.execute(
        ...
        (
            ...
            row["term"],
            ethics_flag,
            "Crossref public metadata record; region is a reviewable figure-associated signal, not a verified event place.",
            now,
        ),
    )
    ...
    conn.execute(
        ...
        (
            record_id,
            loc_id,
            f"Matched term '{row['term']}' in Crossref title/abstract metadata.",
            row["location_confidence"],
            "Crossref metadata region hint for display/review; not a geocoded sighting place.",
        ),
    )
    return record_id
...
def write_report(path: Path, statuses: list[dict[str, Any]], started: str, finished: str, start_count: int, end_count: int, target: int) -> None:
    ...
    lines = [
        "# Crossref Public Metadata Collection Report",
        "",
        "## Execution Context",
        f"- Run: `{ROUND_NAME}`",
        f"- Started: `{started}`",
        ...
        "",
        "## Guardrails",
        "- Crossref records are public metadata, not collected full text.",
        "- Region fields are figure-associated review signals, not verified event places.",
        "- High-noise Yahoo/Cadbury/biomedical/science false positives are skipped.",
        "",
...
def collect(args: argparse.Namespace) -> int:
    ...
    for spec in sorted(TERM_SPECS, key=lambda item: -item.priority):
        if inserted_count >= args.target:
            break
        for item in fetch_items(spec, args.rows, args.max_pages, args.delay):
            candidate = candidate_from_item(spec, item)
